...
class OPTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # hand trans
        init_hand_trans_j = init_joints_3d[21, :3] - init_joints_3d[0, :3]
        hand_trans_weight = np.ones((1, ), dtype=np.float32)
        init_hand_trans = np.concatenate((init_hand_trans, hand_trans_weight)).reshape(1, 4)
        init_hand_trans_j = np.concatenate((init_hand_trans_j, hand_trans_weight)).reshape(1, 4)
        ...
        init_hand_trans = torch.from_numpy(init_hand_trans).float().view(1, 4)
        init_joints_2d = torch.from_numpy(init_joints_2d).float()
        init_joints_3d = torch.from_numpy(init_joints_3d).float()
        init_hand_trans_j = torch.from_numpy(init_hand_trans_j).float().view(1, 4)
